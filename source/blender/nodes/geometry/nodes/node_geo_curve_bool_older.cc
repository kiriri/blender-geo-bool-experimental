 ...
 * start and end.
 */
static bool curve_contains(bke::CurvesGeometry spline, int index, float3 point)
{
 ...
 */
static int count_containing_curves(bke::CurvesGeometry splines, float3 point, int except = -1)
{
  int counter = 0;

  int count = splines.curve_size;
  for (int i = 0; i < count; i++) {
    if ((i != except) && curve_contains(splines, i, point)) {
      counter++;
    }
  }
  return counter;
...
/**
 * Stores a complete map of all connected (evaluated) points and intersections across all involved
 * splines as a double linked list.
 */
struct CurveHull {
  // Used to identify which curve-input we're on. Useful if self intersection is off.
  int curve_id;
  // original spline this point is on.
  blender::bke::CurvesGeometry *spline;
  // CurvesGeometry can contain multiple distinct subsplines.
  long subspline_i;
 ...
 */
static std::vector<CurveHull *> points_to_hull(blender::bke::CurvesGeometry *spline, int curve_id)
{
  std::vector<CurveHull *> result = {};

  blender::Span<float3> points = spline->evaluated_positions();
  bool cycle = spline->cyclic();

  auto curve_len = spline->curve_size;

  for (int i = 0; i < curve_len; i++) {
    auto indices = spline->evaluated_points_for_curve(i);

    //spline->point_data
    //spline->interpolate_to_evaluated() // TODO : Add radius, tile, custom attributes.

    // Create the start point
    CurveHull *start = new CurveHull{.curve_id = curve_id,
                                     .spline = spline,
                                     .subspline_i = i,
                                     .eval_curve_i = indices[0],
                                     .v = 0,
                                     .position = points[indices[0]],
                                     .next = nullptr,
                                     .prev = nullptr};
    CurveHull *prev = start;

    // Now create all the following evaluated points, and reference the start point as .prev
    // Also reference the prev's next as this one.
    // This way we create a linked list.
 ...
 */
static std::list<CurveHull *> splineIntersectAll(std::vector<bke::CurvesGeometry> splines)
{
  SCOPED_TIMER("Spline Intersect All");
  // TODO : Create Binary search tree from bounds (Needs some kind of clumping heuristics. Probably
  // not worth it.)
  ...
  int c = splines.size();
  for (int i = 0; i < c; i++) {
    // for (bke::CurvesGeometry _splines : splines) {

    auto _splines = splines[i];
    auto curve_offsets = _splines.evaluated_offsets();
    Span<float3> evaluated_positions = _splines.evaluated_positions();

    // _splines.bounds_min_max(min,max);

    auto hulls = points_to_hull(&_splines, i);

    for (int j = 0; j < _splines.curve_size; j++) {
      blender::IndexRange indices = _splines.evaluated_points_for_curve(j);

      float3 min = float3(INFINITY, INFINITY, INFINITY);
      float3 max = float3(-INFINITY, -INFINITY, -INFINITY);
      for (int64_t k : indices) {
        minmax_v3v3_v3(min, max, evaluated_positions[k]);
      }
      ...
      int counter = count_containing_curves(o->curve_id == 0 ? splines[1] : splines[0],
                                            o->position);
      o->is_inside = counter % 2 == 1;
      rtree.insert(std::make_pair(bound.Box(), o));
  ...
  while (frontier.size() > 0) {
    ...
    //     } while (next_curve_hull_b != nullptr && current_curve_hull_b->eval_curve_i != 0);
    ...
    for (CurveHull *curve_hull_b : frontier) {
      ...
      //   continue;
      // }
      if (primary_curve->curve_id == curve_hull_b->curve_id || !(bounds[i].intersect(bounds[j]))) {
        continue;
      }
  ...
  for (CurveHull *current_point : results) {
    ...
    bool is_cyclic = current_point->spline->cyclic();
    float curve_length = current_point->spline->points_for_curve(current_point->curve_id).size() +
                         (is_cyclic ? 0 : -1);
    ...
    do {
      ...
      for (Intersection *intersection : intersections) {
        CurveHull *new_point = new CurveHull{
            ...
            .subspline_i = current_point->subspline_i,
            .eval_curve_i = current_point->eval_curve_i,
            .v = original_current_point->v + intersection->v / curve_length,
            .position = math::interpolate(
                             ...
                             float tilt)
{
  ...
  bezierSpline->handle_types_left()[index] = HandleType::BEZIER_HANDLE_FREE;
  bezierSpline->handle_types_right()[index] = HandleType::BEZIER_HANDLE_FREE;
}
...
{
  int resolution = current_point->spline->resolution();

  // control point index + v along segment
  float current_absolute_v = current_point->eval_curve_i / (float)(resolution);
  int current_control_point = (int)(current_absolute_v);

  if (type == CURVE_TYPE_BEZIER) {
    if ((current_point->eval_curve_i % resolution) != 0) {
      return last_intersection_offset;
    }
    BezierSpline *current_spline = (BezierSpline *)current_point->spline; // TODO : This is a bad cast, it's no longer true
    BezierSpline *result_spline = (BezierSpline *)result;

    set_bezier_point(
        result,
        result_index,
        current_spline->evaluated_positions()[current_point->eval_curve_i],
    ...
    blender::bke::CurvesGeometry *current_spline = current_point->spline;
                                 ...
                                 CurveType type)
{
  ...
  if (type == CURVE_TYPE_BEZIER) {
    ...
    float current_absolute_v = current_point->eval_curve_i / (float)(current_resolution);
    int current_control_point = (int)(current_absolute_v);
    float current_relative_v = fmod(current_absolute_v, 1);
    int current_segment_end = (current_control_point + 1) % current_spline->size();
    ...
    float next_absolute_v = other_point->eval_curve_i / (float)(next_resolution);
                                       ...
                                       CurveType return_type)
{
  ...
  int resolution = return_type == CURVE_TYPE_BEZIER ?
                       ((BezierSpline *)start_point->spline)->resolution() :
                       1;

  if (next_point != nullptr) {
    while (current_point != nullptr && (current_point->pass != -2)) {
      if (current_point->intersection == nullptr && (current_point->eval_curve_i % resolution) != 0) {
        current_point = current_point->next;
      ...
      if (current_point->intersection != nullptr)  // current point is a virtual intersection point
      {
        ...
        current_point->pass = -2;
        resolution = return_type == CURVE_TYPE_BEZIER ?
 ...
 */
static std::unique_ptr<Spline> trace_to_spline(std::list<CurveHull *> trace, CurveType return_type)
{
  std::unique_ptr<Spline> result = return_type == CURVE_TYPE_BEZIER ?
  ...
  if (return_type == CURVE_TYPE_BEZIER)  // if cyclical result
  {
 ...
 */
static void trace_hull(bke::CurvesGeometry &results,
                       CurveHull *path,
                       ...
                       CurveType return_type)
{
  ...
  std::vector<std::list<CurveHull *>> valid_traces = {};
  ...
  while (frontier.size() > 0) {
    ...
    if (trace.size() > 0) {
      valid_traces.push_back(trace);
    }
  }

  // TODO : Result, use modified version of trace_to_spline()
}
 ...
 */
static bool is_curve_clockwise(bke::CurvesGeometry *spline, int index)
{

  blender::IndexRange indices = spline->points_for_curve(index);
 ...
 */
static void free_handles(bke::CurvesGeometry splines)
{
  MutableSpan<int8_t> handles_l_t = splines.handle_types_left_for_write();
  MutableSpan<int8_t> handles_r_t = splines.handle_types_right_for_write();
  // Make sure the auto handles are all up-to-date
  splines.calculate_bezier_auto_handles();

  int size = handles_l_t.size();
  for (int i = 0; i < size; i++) {
    handles_l_t[i] = HandleType::BEZIER_HANDLE_FREE;
  }

  size = handles_r_t.size();
  for (int i = 0; i < size; i++) {
    handles_r_t[i] = HandleType::BEZIER_HANDLE_FREE;
  }
}
 ...
 */
static std::unique_ptr<Curves> generate_boolean_shapes(Curves *a,
                                                       Curves *b,
                                                       BoolOperationType type,
                                                       int resolution)
{
  std::unique_ptr<Curves> result = std::make_unique<Curves>();
  bke::CurvesGeometry &result_geometry = bke::CurvesGeometry::wrap(result->geometry);

  // TODO : Copy curves so we can reverse them.

  // Wrap curves geometry as bke::CurvesGeometry, which exposes some helper functions
  bke::CurvesGeometry &splines_a = bke::CurvesGeometry::wrap(a->geometry);
  bke::CurvesGeometry &splines_b = bke::CurvesGeometry::wrap(b->geometry);

  std::vector<bke::CurvesGeometry> splines = {splines_a, splines_b};

  // If all curves involved are bezier curves, we can return a bezier curve as well.
  // If any curve is not a bezier curve, the result must be a poly line.
  CurveType result_type = CURVE_TYPE_BEZIER;
  for (bke::CurvesGeometry _curves : splines) {
    if (!_curves.is_single_type(CURVE_TYPE_BEZIER)) {
      result_type = CURVE_TYPE_POLY;
    }
  }

  // TODO : Eliminate empty curves?
  ...
  for (bke::CurvesGeometry _splines : splines) {
    ...
    free_handles(_splines);

    auto curve_offsets = _splines.evaluated_offsets();
    auto evaluated_positions = _splines.evaluated_positions();

    for (int i = 0; i < _splines.curve_size; i++) {
      // All curves must be clockwise, unless the algorithm turns them explicitly.
      bool is_clockwise = is_curve_clockwise(&_splines, i);

      // Outside curves must go clockwise.
      // Inside curves must go counterclockwise.
      int counter = count_containing_curves(_splines, evaluated_positions[curve_offsets[i]], i);
      if (counter % 2 == (is_clockwise ? 1 : 0)) {
        // curve represents negative space, must go counterclockwise.
        _splines.reverse_curves({i});
      }
    }
  }
  ...
  if (type == BoolOperationType::SUB) {
    splines_b.reverse_curves(IndexMask(splines_b.curve_size));
  }
  ...
  for (CurveHull *path : paths) {
    ...
    for (std::unique_ptr<Spline> &trace : results) {
      if (trace->type() == CURVE_TYPE_BEZIER) {
      ...
      result_geometry.add_spline(std::move(trace));
      length += result_geometry.splines().size();
    }
  }
